import { LinkAdjacencyList } from "./linkAdjacencyList"
...
function hasContour(
  ...
  return (
    round(
      abs(sign(fn(x, y)) + sign(fn(x + dx, y)) + sign(fn(x, y - dy)) + sign(fn(x + dx, y - dy)))
    ) !== 4
  )
}
...
export function computeLinkedLists(
  ...
  function subDivide() {
    ...
    const compute = (vec: readonly [number, number]) =>
      computeLinkedLists(list, fn, vec, newDelta, newDepth, searchDepth, plotDepth)
