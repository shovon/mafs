...
export function get4D<K, V>(map: Map4D<K, V>, [k1, k2, k3, k4]: [K, K, K, K]): V | undefined {
...
export function set4D<K, V>(map: Map4D<K, V>, [k1, k2, k3, k4]: [K, K, K, K], value: V) {
...
export function has4D<K, V>(map: Map4D<K, V>, [k1, k2, k3, k4]: [K, K, K, K]): boolean {
  const m = get2D(map, [k1, k2])
  if (!m) {
    return false
  }
...
export function* iterate4D<K, V>(map: Map4D<K, V>): IterableIterator<[[K, K, K, K], V]> {
...
export function delete4D<K, V>(map: Map4D<K, V>, [k1, k2, k3, k4]: [K, K, K, K]) {
  const m = get2D(map, [k1, k2])
  if (!m) {
    return
  }
